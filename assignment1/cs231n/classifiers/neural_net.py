...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    sum_scores = np.sum(np.exp(s), axis=1, keepdims=True)
    p = np.exp(s)/sum_scores
    ...
    #############################################################################
    #output scores gradient (softmax gradient)
    ...
    grads['b2']= np.sum(dscores, axis=0)*1
    ...
    grads['b1']= np.sum(dhidden, axis=0)*1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    print(train_acc_history,val_acc_history)    
    return {
