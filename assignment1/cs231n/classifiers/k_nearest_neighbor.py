...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #dists[num_test][num_train] = X[num_test] - X_train[num_train]
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    return dists
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      #dists[i] = distance_matrix(X[i], self.X_train)
      #dists[i] =  np.sqrt((np.square(X[:,np.newaxis]-self.X_train).sum(axis=2)))
    ...
    for i in xrange(num_test):
      ...
      if(k<2): 
          ...
          if(c.most_common(2)[0][1]==c.most_common(2)[1][1]):
              y_pred[i] = min(c.most_common(2)[0][0],c.most_common(2)[1][0])
          else:
              y_pred[i] = c.most_common(1)[0][0]
      else:
          y_pred[i] = c.most_common(1)[0][0]
